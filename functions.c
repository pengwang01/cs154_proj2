...
void doBranching(InstInfo ** pipelineInsts, int * stall, int * cycles, int * counter, int *isTaken){
	...
	// j
	if(cur->fields.op == 36){
		if(debug)printf("DEBUG: j to %d\n", cur->fields.imm);
		pc = cur->fields.imm;
		*cycles = *cycles - (cur->fields.imm - *counter);
	}
	// jal
	else if(cur->fields.op == 34){
		regfile[31] = pc;
		if(debug)printf("DEBUG: jal to %d\n", cur->fields.imm);
		pc  = cur->fields.imm;
		*cycles = *cycles - (cur->fields.imm - *counter);
	}

	else{
		cur->s1data = regfile[cur->sourcereg];
		cur->s2data = regfile[cur->targetreg];

		if(cur->fields.rs == cur->fields.rt){
			if(debug)printf("DEBUG: ->>>>>>>>>>cur rs = rt\n");
			//pc += cur->fields.imm;
			//*cycles = *cycles - cur->fields.imm;
			*isTaken = 1;
		}
		else if(cur->signals.depPreExec || cur->signals.depPrePreExec || cur->signals.depPreMem){
			if(cur->signals.depPreExec == 1){
				cur->s1data = pre->aluout;
			}
			// if cur rt depends on pre exec
			...
			if(debug)printf("DEBUG: s1data: %d, s2data: %d\n", cur->s1data, cur->s2data);
			cur->aluout = cur->s1data - cur->s2data;
			if(cur->aluout >= 0 ){
				*isTaken = 1;
				//pc = pc + cur->fields.imm;
				//*cycles = *cycles - cur->fields.imm;
			}
