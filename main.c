...
void doStage(int isFirst, int *stall, int * cycles, int * counter, int *isTaken);
...
int main(int argc, char *argv[])
{
	...
	do{
		doStage(needsFetch, &stall, &cycles, &count, &isTaken);	// doStage calls stage functions
		...
		// if stall is being set
			...
			if(isTaken){
				...
				pipelineInsts[1] = saved;				// stall the execute stage
				clearInst(pipelineInsts[1]);
				isTaken = 0;								// reset stall flag
				cycles++;
	...
	if(!*stall){
		...
		if(pipelineInsts[1]->inst!=0){
			...
			if(pipelineInsts[1]->signals.btype != 0){
				...
				doBranching(pipelineInsts, stall, cycles, counter, isTaken);
			}
		}
	}
	if(pipelineInsts[0]->inst!=0 || isFirst){
		if(debug)printf("DEBUG: fetching\n");
		if(debug)printf("DEBUG: isTaken = %d\n", *isTaken);
		fetch(pipelineInsts[0]);
		if(*stall || isTaken){		// when stall is being set, needs to defetch one instruction
			/*if(pipelineInsts[1]->signals.btype == 2){
...
void setDependency(InstInfo ** pipelineInsts, int *stall){
	...
	// pre inst is not 0 and write to register
